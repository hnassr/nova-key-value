<template>
    <DefaultField :field="currentField" :errors="errors" :full-width-content="mode === 'modal'" :show-help-text="showHelpText">
        <template #field>
            <draggable v-model="rows" :disabled="false" class="list-group justify-center" ghost-class="ghost-card" :item-key="guid()">
                >
                <template #item="{ element }">
                    <div
                        :ref="element.key"
                        class="flex cursor-move mt-3 px-6 bg-white rounded-lg border border-gray-200 shadow-md hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700"
                    >
                        <div class="w-3/4 py-3 mr-1 ml-12">
                            <select
                                v-if="currentField.selectableKeys"
                                v-model="element.key"
                                class="form-control form-input-bordered w-full text-center"
                                :class="errorClasses"
                                placeholder="Aggregators"
                            >
                                <option v-for="(description, value, index) in currentField.selectableKeys" :key="index" :value="value">
                                    {{ description }}
                                </option>
                            </select>
                            <input
                        ...
                        <div class="w-3/4 py-3">
                            <select
                                v-if="currentField.selectableValues"
                                v-model="element.value"
                                class="form-control form-input-bordered w-full text-center"
                                :class="errorClasses"
                                placeholder="Value"
                            >
                                <option v-for="(description, value, index) in currentField.selectableValues" :key="index" :value="value">
                                    {{ description }}
                                </option>
                            </select>
                            <input
...
export default {
    ...
    watch: {
        rows: {
            handler(value) {
                value = _.filter(value, function (row) {
